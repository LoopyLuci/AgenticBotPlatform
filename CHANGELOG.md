...
- **Every bot now has a real backend-level fallback, OpenCode then Hermes CLI then the
  ...
  with just its own `backend` set (the common case) had no fallback at all.
  `config/backends.yaml`'s new top-level `backend_backup: [opencode, hermes_cli,
  hermes_gateway]` is now appended to every resolved routing chain
  (`Router.resolve_chain`/`_with_global_backup`), so a down/misconfigured/out-of-quota
  primary backend now falls through those three, in order, before giving up — matching
  the standing "never Claude by default" instruction (a fallback the router reaches for
  on its own follows the same rule a default does). See `README.md`'s "How routing
  works".
