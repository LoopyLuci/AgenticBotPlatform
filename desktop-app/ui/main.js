...
// copy confirmation only, with background:var(--panel, #222)/color:var(
// --fg, #fff), NEITHER of which is a real token this app's design system
// defines (the real names are --surface-3/--ink/etc, see the :root
// blocks in index.html) — every toast silently rendered at its literal
// fallback color, never adapting to the light theme. Generalized so the
// many `catch (_e) { return; }` sites across this file that silently
// no-op on a failed refresh/action can surface something to the operator
// instead of freezing without explanation. Supports stacking (each toast
// is its own element, oldest on top) since more than one thing can
// legitimately fail/succeed close together.
...
async function refreshBots() {
  ...
  // Every mutating bot-lifecycle action below shares the same guard
  // data-bot-newsession already uses individually (disabled + try/catch)
  // — no disable-while-pending meant a fast double-click could fire two
  // mutating requests before the grid re-rendered, and no try/catch
  // meant a thrown error (401, network blip) was an unhandled promise
  // rejection: refreshBots() never ran, the button never recovered any
  // state, and the user saw literally nothing happen with no
  // explanation. _guardedBotAction wraps both fixes in one place so each
  // handler below stays a one-liner.
...
(function initSidebarCollapse() {
  ...
  side.querySelectorAll('nav.sidenav a[href^="#"]').forEach((a) => {
    a.addEventListener('click', (e) => {
      ...
      // sit in between (see index.html's comment on the section rule).
      // Force every section to its real layout right before scrolling,
      // then let content-visibility:auto resume optimizing off-screen
      // ones once we've actually arrived.
      const sections = document.querySelectorAll('section');
      sections.forEach((sec) => { sec.style.contentVisibility = 'visible'; });
