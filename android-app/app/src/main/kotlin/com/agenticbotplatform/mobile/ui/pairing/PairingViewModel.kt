...
import com.agenticbotplatform.mobile.data.PairingPayload
...
class PairingViewModel @Inject constructor(
    ...
    fun onScanned(raw: String) {
        val payload = repository.parse(raw) ?: run {
            _state.value = PairingState.Error("That QR doesn't look like an Agentic Bot Platform pairing code.")
            return
        }
     ...
     * agenticbotplatform://pair link tapped from a share or fired by the desktop
     ...
     * "agenticbotplatform://pair?..." text verbatim, since it's the exact same
