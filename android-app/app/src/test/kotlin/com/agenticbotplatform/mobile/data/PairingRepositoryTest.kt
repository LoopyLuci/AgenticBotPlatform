...
class PairingRepositoryTest {
    ...
    fun `a full pairing URI with both hosts parses every field`() {
        val result = repository.parse("agenticbotplatform://pair?host=192.168.1.5&host2=my-tailnet.ts.net&key=abc123")
    ...
    fun `a pairing URI with all three hosts parses host3 (the Funnel URL) too`() {
        val encoded = "agenticbotplatform://pair?host=192.168.1.5&host2=100.101.98.77%3A8787" +
    ...
    fun `a pairing URI with only a host and key parses host2 as null`() {
        val result = repository.parse("agenticbotplatform://pair?host=192.168.1.5&key=abc123")
    ...
    fun `a pairing URI missing the key entirely fails to parse`() {
        val result = repository.parse("agenticbotplatform://pair?host=192.168.1.5")
    ...
    fun `a percent-encoded host in the query string is decoded`() {
        val result = repository.parse("agenticbotplatform://pair?host=my%20server&key=abc123")
