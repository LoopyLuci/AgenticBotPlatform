...
| Terminal TUI (`python -m bot.tui`, Textual) and a scriptable CLI (`python -m abp_cli`) | **Partly built — second parity phase.** Both cover bots (full CRUD + lifecycle + schedules), real chat (`send-to-bot`), the full `native_agent.*` agent-config schema, per-bot agent settings, provider management, swarms, sessions, and the SSH Toolkit (see below). The CLI additionally covers the terminal panel (as a slash-command dispatcher, not a raw shell), hooks, plugins, skills, MCP (internal and external), security & allowed-users, devices/mobile keys, snapshots, env/config/diagnostics, peers/federation and kanban — real and tested against the live app, just without a TUI screen yet. Still GUI-only: Android push/mobile-key QR pairing UI, and the Models page's usage/limit screens beyond provider/model browsing. See [cli-tui.md](cli-tui.md) |
| SSH Toolkit ([github.com/LoopyLuci/SSH_Toolkit](https://github.com/LoopyLuci/SSH_Toolkit)) — a separately maintained PowerShell tool for managing/visualizing SSH connections between machines | Built, vendored as a git submodule (`vendor/ssh_toolkit`) reached only through `bot/ssh_toolkit.py`. In the dashboard GUI, the desktop app, the TUI (`SshToolkitScreen`, press `h`), and the CLI (`abp_cli ssh ...`) alike, all against the same Python wrapper — add/list/remove/test a connection, run a remote command, the proxy-jump graph, and update check/apply. A machine-wide `auto_update` setting (`never`/`notify`/`auto`, default `never`) drives an every-6-hours background check that can notify-only or apply the update itself; the setting never affects this machine's own `~/.ssh/config` or connection registry, only the vendored submodule's own files. Tested end to end against the real, published toolkit (not mocked) |
